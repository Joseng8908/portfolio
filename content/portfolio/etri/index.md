---
title: "P/D Disaggregated Serving in Backend.AI"
summary: "Backend.AI 기반 이기종 가속기(GPU + NPU) P/D Disaggregated Serving"
description: "AIDC Portfolio"
...
GPU(prefill)랑 NPU(decode)를 분리한 이기종 파이프라인으로 Llama-3.3-70B를 서빙하고, goodput이 안 나오는 병목이 연산(TPOT)이 아니라 요청 파이프라인(TTFT)에 있다는 걸 벤치마킹을 통해 알아냈다.
...
Backend.AI를 적용(kubernetes와 같은 오케스트레이팅 플랫폼)을 도입하기 위해 컨테이너화를 진행하면, 자원 격리 측면에서 병목이 생길 수 있다는 것을 알아냈다.

## 배경 — 왜 P/D를 분리하나

LLM 추론은 두 단계의 성격이 완전히 다르다. 입력 전체를 한 번에 처리하는 **prefill**은 연산 집약적이라 연산 성능 높은 GPU가 유리하고, 토큰을 하나씩 뽑아내는 **decode**는 메모리 대역폭과 관련이 있기때문에 GPU/NPU 상관 없지만, 비용적 측면에서 NPU가 이점이 있기에 **decode**부분에서는 NPU를 사용하였다.
...
- **플랫폼에 신규 NPU 편입**: n300 전제로 짜여 있던 가속기 플러그인을 신규 카드도 인식하도록 패치하고, 자원 슬롯 등록. 5건 넘는 플랫폼 이슈를 하나씩 수정해서 NPU 4장을 정상 인식시켰다.
...
- 컨테이너 하에서 P/D disaggregated serving(GPU/NPU)를 서빙하는 것이 가능함을 증명하였다.(성능적 측면을 고려하지 않았지만, 컨테이너화를 하고 세부 설정을 하는 것이 가능하다는 것은, 스케일링/스케줄링/자가복구 등등의 기능을 kubernetes와 같은 오케스트레이팅 도구를 이용해 적용을 할 수 있다는 증거이다)
- 2모델 × 4워크로드 × 8요청률 = **64측정점** goodput 벤치마크를 돌리고, goodput 제약이 TPOT가 아니라 **TTFT에 병목**에서 온다는 걸 알아냈다.
- 컨테이너로 돌릴 때 생기는 성능 오버헤드(TPOT, TTFT의 지연)의 원인을 `schedstat` 기반으로 실측했다 .
...
- **네트워크 격리가 분산 추론의 병목이다.** 컨테이너 네트워크 모드 하나가 RDMA KV 전송의 성패를 갈랐다. "컨테이너 안에서 보는 자기 주소 ≠ 밖에서 도달 가능한 주소"라는 NAT 격리의 함정을 직접 밟으면서 이해했다.
- 이기종 서빙의 진짜 난점은 개별 가속기 성능이 아니라 그것들을 **잇는 경로**(KV 전송, 네트워크, 스케줄링)에 있다는 걸 체감했다.
- 컨테이너화를 진행하면서 워크로드나 가속기에 맞는 세팅, 현재 서버의 스펙 등을 고려해야 제대로 된 성능을 뽑아낼 수 있음을 알아냈다.

## Future Work
- 현재 NPU가 컨테이너 하에서 돌아갈 때 CPU를 사용하는 것은 관찰했지만, 그 이유를 규명하지 못하였다.
- GLM 5.2는 RTX Pro 6000ada를 사용했는데 아직 sm120을 vLLM쪽에서 지원하지 못했었다.
